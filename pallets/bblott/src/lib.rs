...
pub struct LotteryConfig<BlockNumber> {
	/// Price per entry.
	// price: Balance,
...
pub mod pallet {
	...
	#[pallet::storage]
	pub(crate) type Lottery<T: Config> = StorageValue<_, LotteryConfig<T::BlockNumber>>;
	...
	impl<T: Config> Pallet<T> {
		...
		///
		/// Total of each users bets on each call needs to be recorded in a key-value 
		...
		pub fn buy_ticket(
			origin: OriginFor<T>,
			// cal: Box<<T as Config>::RuntimeCall>,
			amount: BalanceOf<T>,
		) -> DispatchResult {
			let caller = ensure_signed(origin.clone())?;
			// cal.clone().dispatch(origin).map_err(|e| e.error)?;
			let price = amount;

			// let _ = Self::do_buy_ticket(&caller, &cal);
			let _ = Self::do_buy_ticket(&caller, &price);
			Ok(())
		}
		...
		pub fn start_lottery(
			...
			Lottery::<T>::try_mutate(|lottery| -> DispatchResult {
				...
				*lottery = Some(LotteryConfig { start, length, delay, repeat });
		...
		pub fn stop_lottery(origin: OriginFor<T>) -> DispatchResult {
			...
			// Not much we can do if this fails...
			let res =
				T::Currency::transfer(&Self::account_id(), &winner, lottery_balance, KeepAlive);
			debug_assert!(res.is_ok());
			...
			// Kill the lottery storage.		
			Lottery::<T>::mutate(|lottery| {
				*lottery = None;
			});
			// We choose not need to kill Participants and Tickets to avoid a large
			// number of writes at one time. Instead, data persists between lotteries,
...
impl<T: Config> Pallet<T> {
	...
	fn do_buy_ticket(
		caller: &T::AccountId,
		cost: &<<T as Config>::Currency as Currency<T::AccountId>>::Balance,
	) -> DispatchResult {
		...
		Participants::<T>::try_mutate(
			...
			|(lottery_index, participating_calls)| -> DispatchResult {
				...
				T::Currency::transfer(caller, &Self::account_id(), *cost, KeepAlive)?;
				// Create a new ticket.
